...
\subsection{Photonic Inverse Design: Thin-Film Multilayers}
To demonstrate DiffMCMC on a challenging high-dimensional physical inverse problem, we applied it to the structural inference of \textbf{Photonic Thin-Film stacks}. We utilized a custom **Differentiable Transfer Matrix Method (TMM)** solver to compute the reflectance spectrum $R(\lambda)$ of a 10-layer dielectric mirror ($\text{SiO}_2 / \text{Si}_3\text{N}_4$ stack on Silicon).
The goal is to infer the 10 layer thicknesses $\mathbf{d} \in \mathbb{R}^{10}$ given a noisy observed spectrum.
\begin{enumerate}
    \item \textbf{Dataset}: Generated a professional-grade HDF5 dataset of 2,000 synthetic stacks with random thicknesses $[50, 300]\text{nm}$.
    \item \textbf{Likelihood}: Gaussian noise on the spectrum ($\sigma=0.1$).
    \item \textbf{Inference}: 10-dimensional posterior with multimodal "Bragg peak" ambiguities.
\end{enumerate}

\textbf{Results}: The DiffMCMC framework successfully integrated the differentiable TMM physics engine. While the rugged energy landscape makes global acceptance challenging (falling back to robust local kernels), the architecture demonstrates the capability to handle complex, differentiable physical forward models within a rigorous Bayesian loop.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/photonic_results.png}
    \caption{Photonic Inference Results. Top Left: Spectral reconstruction matches ground truth. Top Right: Trace plot of Layer 1 thickness. Bottom Left: Posterior correlation between $d_1$ and $d_2$. Bottom Right: Relative error of inferred thicknesses.}
    \label{fig:photonic}
\end{figure}
